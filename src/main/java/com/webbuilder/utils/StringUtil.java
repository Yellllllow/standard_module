...
public class StringUtil {
	 ...
	 * @throws Exception
	 */
	public static String replaceParametersForSql(HttpServletRequest request,
			String text) throws Exception {
		int start = 0, startPos = text.indexOf("[{#", start), endPos = text
				.indexOf("#}]", startPos + 2);
		if (startPos != -1 && endPos != -1) {
			...
			while (startPos != -1 && endPos != -1) {
				paramName = text.substring(startPos + 3, endPos);
				prefix = substring(paramName, 0, 4);
				if (prefix.equals("Var.")) {
					paramValue = Var.get(paramName.substring(4));
					if (paramValue == null)
					...
					paramValue = WebUtil.fetch(request, paramName);
				if(paramValue.isEmpty() || paramValue.equals("null")){
					return "";
				}
				buf.append(text.substring(start, startPos));
				buf.append(paramValue);
				start = endPos + 3;
				startPos = text.indexOf("[{#", start);
				endPos = text.indexOf("#}]", startPos + 3);
			}
			buf.append(text.substring(start));
			return buf.toString();
		} else
