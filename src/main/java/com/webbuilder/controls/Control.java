...
public class Control {
	...
	protected String gs(String name) throws Exception {
		String result=xwlObject.optString(name);
		if(name.equals("value") && !result.isEmpty()){
			int index=result.indexOf("[{#");//拦截可为空的SQL参数
			if(index != -1){
				return StringUtil.replaceParametersForSql(request, result);
			}
		}
