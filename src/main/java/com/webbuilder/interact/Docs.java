...
public class Docs {
	...
	public static void getFileContent(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		File base = new File(Main.path, "webbuilder/docs"), file;
		String dir = request.getParameter("dir");

		if (dir.startsWith("@"))
			file = new File(base, dir.substring(1));
		else
			file = new File(dir);
		if (!FileUtil.isAncestor(base, file))
			throw new Exception("Access denied.");
		WebUtil.response(response, FileUtil.readUtfText(file));
	}
	...
	public static void getTopicTree(HttpServletRequest request,
		...
		StringBuilder buf = new StringBuilder();
		File base, file, root = new File(Main.path, "webbuilder/docs");
		JSONArray ja;
		JSONObject jo;
		int i, j;
		...
		if (StringUtil.isEmpty(dir))
			...
			base = new File(dir);
			if (!FileUtil.isAncestor(root, base))
				throw new Exception("Access denied.");
		}
		...
		for (i = 0; i < j; i++) {
			...
			if (!FileUtil.isAncestor(root, file))
				throw new Exception("Access denied.");
