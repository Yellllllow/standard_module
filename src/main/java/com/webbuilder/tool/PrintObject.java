...
public class PrintObject {
	...
	public static String preview(JSONArray data, JSONArray meta, String title,
		...
		for (i = 0; i < j; i++) {
			...
			for (x = 0; x < y; x++) {
				...
				//去除特定列[weekData,monthData]
				String hours="weekData,monthData";
				if(hours.indexOf(names[x])!=-1){
					continue;
				}
				boolean is_special =false;//特殊处理标识[:]
				if(value.indexOf(":")!=-1 && weeks.indexOf(names[x])!=-1){
					is_special=true;
					//System.out.println(Arrays.deepToString(value.split(":")));
				...
				if(is_special){
					buf.append("<td height='70px' align=\"");
				}else{
					buf.append("<td align=\"");
				}
				buf.append(aligns[x]);
	...
	private static String createHeaders(ArrayList<JSONObject> children,
		...
		StringBuilder buf = new StringBuilder();
		//去除特定字段
		String exclude = "create_by, update_by";
		...
		for (x = 0; x < maxDepth; x++) {
			...
			if (Var.getBool(printPath + "headerFontBold"))
				buf.append(";font-weight:bold");
			buf.append("\">");
			if (x == 0 && numText != null && !numText.endsWith("(系统)")) {
				buf.append("<td align=\"right\"");
				if (maxDepth > 1) {
					buf.append(" rowspan=\"");
					buf.append(maxDepth);
			...
			for (JSONObject item : children) {
				String di=item.optString("dataIndex");
				if(exclude.indexOf(di)!=-1){
					continue;
				}
