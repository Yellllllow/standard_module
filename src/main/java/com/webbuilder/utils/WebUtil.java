...
public class WebUtil {
	...
	public static boolean checkRight(HttpServletRequest request, String xwlId)
		...
		//不验证developer用户权限   modify by team 2015/04/04
		if (StringUtil.indexOf(userRoles, "developer") != -1)
			return true;
		while (d != null) {
			setRoles = d.roles;
			if (setRoles != null) {
				for (String r : setRoles) {
					...
					//不验证developer角色权限   modify by team 2015/04/04
					if (role.equals("developer")
							|| StringUtil.indexOf(userRoles, role) != -1)
						return true;
					if (StringUtil.indexOf(userRoles, role) != -1)
						return true;
				}
				return false;
			}
	...
	public static boolean checkLogin(HttpServletRequest request,
		...
		//验证单点登录
		if (Session.casverify(request, response)) {
			return true;
		}
		if (session == null || session.getAttribute("sys.logined") == null) {
			...
			request.setAttribute("sys.ssoId", session
					.getAttribute("sys.ssoId"));
	 ...
	 * @throws Exception
	 */
	public static void writer(HttpServletResponse response, String obj) throws Exception {
