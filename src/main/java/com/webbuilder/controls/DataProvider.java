...
public class DataProvider extends FrontControl {
	...
	public void create() throws Exception {
		...
		if (mapFields.isEmpty()) {
			String km = request.getParameter("__keyMap");
			if (km == null)
				keyMap = null;
			else
				keyMap = new JSONObject(km);
		} else
		...
		if (obj instanceof ResultSet)
			...
			WebUtil.response(
					response,
					StringUtil
							.concat("{\"total\":1,\"metaData\":{\"fields\":[{\"name\":\"result\",\"type\":\"string\"}]},\"columns\":[{\"dataIndex\":\"result\",\"header\":",
									StringUtil.quote(Str.format(request,
											"result")),
	...
	private void outputJsonArray(ResultSet rs, Integer total,
		...
		if (gb("createColumns", false)) {
			buf.append(",\"columns\":");
			buf.append(DbUtil.getColumns(meta, j, gs("editorType"),
					gb("rowNumber", false), dateAsString, keyMap));
		}
		if (isTree)
			buf.append(",\"children\":[");
		else
