...
public class MngTool {
	 ...
	 * @throws Exception
	 */
	public static void updateTKey(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String pid=request.getAttribute("oldKey.key_id").toString();
		String type=request.getParameter("key_type_t").toString();
		ResultSet rs=DbUtil.query(request, "select key_type,key_name,key_value from wb_key where key_pid='"+pid+"' and key_is_leaf =1");
		while(rs.next()){
			SysMap.remove(rs.getString("key_type"),rs.getString("key_name"));
			SysMap.put(type, rs.getString("key_name"), rs.getString("key_value"));
		}
	}
	...
	public static void getPwdMD5Str(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String pwd = request.getAttribute("pwd").toString();
		request.setAttribute("md5_str", Encrypter.getMD5(pwd));
	}
	 ...
	 * @throws Exception
	 */
	public static void setRoleModules(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String id = request.getParameter("roleId"), moduleId;
		//if (!XwlBuffer.exists(id))
		//	WebUtil.notExist(request);
		ResultSet rs1=DbUtil.query(request, "select * from wb_role where ROLE_ID='"+id+"'");
		rs1.next();
		String roleName=rs1.getString("ROLE_NAME");
		JSONArray moduleIds = new JSONArray(request.getParameter("moduleIds"));
		int i, j;
		PreparedStatement st1 = null, st2 = null;
		Connection conn = DbUtil.getConnection(request);
		ArrayList<String> list;
		try {
			ConcurrentHashMap<String, XwlData> map = XwlBuffer.getXwlMap();
			//判断缓存模块里是否有该角色,有则排除
			String notInIds="SELECT a.MODULE_ID from wb_module a";
			ResultSet rs2=DbUtil.query(request, notInIds);
			while (rs2.next()) {
				XwlData data = map.get(rs2.getString("MODULE_ID"));
				ArrayList<String> listRoles=new ArrayList<String>();
				if(data != null){
					listRoles=data.roles;
				}
				if(listRoles != null && listRoles.size() >0){
					ArrayList<String> new_listRoles=new ArrayList<String>();
					for (String string : listRoles) {
						if(!string.equals(id+"="+roleName)){
							new_listRoles.add(string);
						}
					}
					data.roles=new_listRoles;
				}
			...
			if (j > 0) {
				st2 = conn
						.prepareStatement("insert into WB_MODULE_ROLE (MODULE_ID,ROLE_ID) values(?,?)");
				for (i = 0; i < j; i++) {
					moduleId = moduleIds.optString(i);
			...
			list = new ArrayList<String>();
			for (i = 0; i < j; i++) {
				XwlData data = map.get(moduleIds.optString(i));
				list=data.roles;
